...
private
  ...
  def find_for_ouath(provider, access_token, resource=nil)
    ...
    puts "ACCESS TOKEN #{access_token.to_yaml}"
    ...
    auth = user.services.find_by_provider(provider)
    if auth.nil?
      auth = user.services.build(:provider => provider)
      user.services << auth
    end
  ...
  def find_for_oauth_by_uid(uid, resource=nil)
    user = nil
    if auth = Service.find_by_uid(uid.to_s)
      user = auth.user
    end
    return user
  end
